...
_DRAIN_BOUNDARY_STATE = {"last_boundary_merge_at": None, "arrivals_absorbed_last_pass": 0}
...
def _rebalance_claim(owned: list, budget: int, path: str, proc_path: str) -> tuple:
    """Merge newly queued entries into the remaining claim and re-sort.

    ``owned`` is the not-yet-run remainder, in run order. Returns
    ``(new_owned, absorbed, displaced)``. Displaced entries are handed to the
    caller to defer ONCE at the end of the pass — see the note below on why
    writing them here corrupts #261's starvation accounting.

    ``.processing`` is rewritten to everything the pass still HOLDS — the
    remainder plus anything displaced and not yet written back — so #244's
    recovery folds back what is still owed and nothing already run.
    """
    if budget <= 0:
        _write_processing(proc_path, owned)
        return [], 0, list(owned)

    arrivals: list = []
    merge_path = proc_path + ".merge"
    try:
        # Atomic: a /mine POST landing mid-merge goes to a fresh `path` and is
        # picked up by the NEXT boundary rather than being read and lost.
            ...
            with open(merge_path, encoding="utf-8") as f:
                for ln in f:
                    if not ln.strip():
                        continue
                    try:
                        arrivals.append((ln, json.loads(ln)))
    ...
    # end of the pass; `.processing` carries them meanwhile so a crash still
    # recovers them (#244).
    _write_processing(proc_path, keep + displaced)
    ...
    _DRAIN_BOUNDARY_STATE["arrivals_absorbed_last_pass"] += absorbed
...
async def _drain_pending_mines() -> int:
    ...
    try:
        ...
        return 0
    count = 0
    _DRAIN_BOUNDARY_STATE["arrivals_absorbed_last_pass"] = 0
        ...
        while owned:
            ...
            try:
                ...
                owned, _absorbed, _displaced = _rebalance_claim(
                    owned, _cap - consumed, path, proc_path
                )
...
# ── Search routes (palace-daemon#101 slice #3) ────────────────────────────────
# The five MCP-backed search endpoints (/search, /search/hybrid,
# /search/keyword, /search/age-fused, /context) live in search_routes.py.
# Their handlers resolve core symbols via ``import main`` at request time,
# so test patches on main._call / main._load_age_extractor / main._mp /
# main._rerank stay effective. _search_args (above) and _load_age_extractor
# (below) remain here — the extracted handlers call them as
# main._search_args / main._load_age_extractor.
from search_routes import router as _search_router  # noqa: E402
app.include_router(_search_router)
# Re-export the handlers the test suite invokes directly
# (test_search_age_fused_hydration, test_search_hybrid_fusion_mode).
from search_routes import search_age_fused, search_hybrid  # noqa: E402,F401
...
# ── Backfill-age routes (palace-daemon#101 slice) ─────────────────────────────
# POST /backfill-age + GET /backfill-age/status live in backfill_routes.py now,
# along with their shared run-state (_backfill_state / _backfill_lock) and the
# _backfill_unprocessed_breakdown query helper. The handlers resolve core
# symbols via ``import main`` at request time, so test patches on
# main._check_auth / main._check_viz_auth / main._mp stay effective.
from backfill_routes import router as _backfill_router  # noqa: E402
app.include_router(_backfill_router)
# Re-export the pure breakdown helper the test suite invokes directly
# (tests/test_backfill_unprocessed.py calls main._backfill_unprocessed_breakdown).
from backfill_routes import _backfill_unprocessed_breakdown  # noqa: E402,F401
