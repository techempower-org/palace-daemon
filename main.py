...
@app.get("/dump_drawers")
async def dump_drawers(
    ...
    """Paginated raw-drawer dump for migration tools.

    Returns ids + documents + metadatas (and embeddings when requested)
    straight from the cached chromadb collection — the same client that
    /search and the MCP layer use. This sidesteps the chromadb 1.5.x
    SIGSEGV class that bites fresh ``PersistentClient(path=…)`` opens
    against long-lived palaces (see chroma-core/chroma#6949): the daemon
    holds a working client in memory, and downstream migration tooling
    that talks to this endpoint inherits the daemon's working state
    instead of paying for a fresh-open crash + multi-hour repair.

    Page with ``limit`` / ``offset``; iterate until the response's
    ``count`` is less than ``limit``.

    ``include_embeddings=true`` (default) makes the response heavy
    (~1.5 KB per drawer for 384-dim float32). For metadata-only
    pagination, set it to false.
    ...
    def _fetch():
        # Reach directly into the cached chromadb client. _get_collection
        # is the same function the daemon uses for every other read path,
        # so this inherits all the in-memory state (HNSW index, metadata
        # cache, embedding function) that fresh PersistentClient opens
        # don't get.
        client = _mp._get_collection()._client if hasattr(_mp._get_collection(), "_client") else None
        # The collection cache _is_ the working collection for the
        # default collection; for an alternate collection name we
        # resolve via the chromadb client.
        default_name = _mp._config.collection_name
        if collection == default_name:
            col = _mp._get_collection()
        else:
            # Resolve a non-default collection via the same client the
            # default uses. mempalace's PersistentClient sits behind a
            # ChromaCollection wrapper; reach in for the raw client.
            raw = getattr(_mp._get_collection(), "_collection", None)
            if raw is None:
                return None, f"cannot resolve raw collection from {type(_mp._get_collection()).__name__}"
            try:
                col = raw._client.get_collection(collection)
            except Exception as e:
                return None, f"collection {collection!r} not found: {e}"
        try:
            batch = col.get(limit=int(limit), offset=int(offset), include=include)
        except Exception as e:
            return None, f"col.get failed: {e}"
        return batch, None

    loop = asyncio.get_running_loop()
    batch, err = await loop.run_in_executor(None, _fetch)
    if err is not None:
        raise HTTPException(status_code=500, detail=err)

    # Normalize embeddings to plain Python lists (chromadb returns numpy
    # arrays). Migration consumers handle list[list[float]] more reliably
    # than ndarray across HTTP boundaries.
