...
Usage:
  python monitor.py                          # defaults: http://localhost:8086, 5s interval
  python monitor.py --url http://artemis:8086 --interval 3 --log palace-monitor.log
  python monitor.py --url http://artemis:8085 --interval 10   # watch production
"""
...
def poll(base_url: str, api_key: str) -> Snapshot:
    ...
    # /stats returns {graph: {total_rooms,...}, status: {total_drawers,...}, kg: {current_facts,...}}
...
def render(cur: Snapshot, prev: Optional[Snapshot], alerts: list[Alert],
    ...
    if not cur.reachable:
        lines.append(f"{ts_label}  {_c(RED + BOLD, '✗ UNREACHABLE')}")
    else:
        ...
        base_k = baseline.kg_facts    if baseline else None

        drawer_str = str(cur.drawer_total) if cur.drawer_total >= 0 else "?"
        kg_str     = str(cur.kg_facts)     if cur.kg_facts >= 0     else "?"

        if base_d is not None and cur.drawer_total >= 0:
            baseline_note = _c(DIM, f"  baseline {base_d}")
        else:
            baseline_note = ""
...
def main():
    ...
    p.add_argument("--url",       default="http://localhost:8086", help="Daemon base URL")
