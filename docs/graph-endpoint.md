...
> cites.
>
> Live verification 2026-04-25: `/graph` returns 200 in ~34s on the
> first hit (cold), <1s warm, full payload (36 wings, 68 rooms,
...
Add to `palace-daemon/main.py`, mirroring the `/stats` pattern at
`main.py:452-461`. Bumps daemon version 1.5.1 → 1.6.0 (minor, additive).
